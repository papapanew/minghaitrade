...
class WxPayApi
{
    /**
     * 检查微信支付配置是否存在
     * @return bool 配置存在且有效返回true，否则返回false
     * @throws WxPayException 如果配置不存在或无效，抛出异常
     */
    private static function checkWxPayConfig()
    {
        // 使用ThinkPHP的数据库类进行查询
        $result = false;
        if (class_exists('\\think\\Db')) {
            ...
            if (!empty($PayConfig)) {
                ...
                if (empty($PayConfig['is_open_wechat']) && !empty($PayConfig['appid']) && !empty($PayConfig['mchid']) && !empty($PayConfig['key'])) {
                    $result = true;
                }
            }
        }
        ...
        if (!$result) {
            throw new WxPayException('微信支付配置不存在或未正确配置，请检查微信支付配置');
        }
    ...
    /**
     * 
     * 测速上报，该方法内部封装在report中，商户无需直接调用，使用时请注意异常流程
     ...
     * @param string $msg 如果回调失败，返回失败原因
     * 直接回调函数使用方法: notify($config, you_function, $msg);
     * 回调类成员函数方法: notify($config, array($this, you_function), $msg);
     * $callback原型为：function function_name($data){}
     */
    ...
    {       
        ...
        curl_setopt($ch,CURLOPT_SSL_VERIFYPEER, FALSE); // 不验证https的安全性 by 陈风任
