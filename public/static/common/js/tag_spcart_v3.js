...
function CartUnifiedAlgorithm(is_sold_out = null ,aid = null, symbol = null, selected = null, spec_value_id = null, cart_id = null){
    ...
    if (Number(is_sold_out) < Number(CartNum)) {
        layer.msg(ey_foreign(ey_foreign_users51, is_sold_out), {time: 1500});
    ...
    var TotalCartNumberV = $('#TotalCartNumber');   //购物车总数
    var TotalNumberV = $('#TotalNumber');           //选中总数
    ...
    if ('change' == symbol) {
        ...
        if ('1' > NumV.val() || '' == NumV.val()) {
            NumV.val(1);
            layer.msg(ey_foreign(ey_foreign_users52, 1), {time: 1500});
        }
        // 计算单品小计
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            if ('0' == res.code) {
                ...
                TotalCartNumberV.html(res.data.CartAmountVal);
...
function Checked(cart_id,selected){
    ...
    // html无刷新更新选中状态
    var TotalNumberV = $('#TotalNumber'); // 获取选中对象总数
    ...
    if (JsonData.is_wap && 1 == JsonData.is_wap) {
        if (0 < NumberVal) {
            $('#SubmitOrder_v455820').show();
            $('#SubmitOrder_0_v455820').hide();
        } else {
            $('#SubmitOrder_v455820').hide();
            $('#SubmitOrder_0_v455820').show();
        }
    }
        ...
        $.ajax({
            ...
            success:function(res){
                if (1 == res.code) {
                    ...
                    var CartNumberVal    = parseInt($('#TotalCartNumber').text()); // 初始化参数
                    var num = Number($('#'+cart_id+'_num').val());
                    $('#TotalCartNumber').html((CartNumberVal-num));
...
// 批量删除购物车产品
function BatchCartDel(){
    var a = [];
    $('input[name^=ey_buynum]:checked').each(function(i,o){
        a.push($(o).attr('cart-id'));
    })
    if(a.length == 0){
        ...
        $.ajax({
            ...
            success:function(res){
                if (1 == res.code) {
                    layer.msg(res.msg, {time: 1000});
                    var CartNumberVal    = parseInt($('#TotalCartNumber').text()); // 初始化参数
                    $('input[name^=ey_buynum]:checked').each(function(index, item){
                        cart_id = $(item).attr('cart-id');
                        var num = Number($('#'+cart_id+'_num').val());
                        $('#TotalCartNumber').html((CartNumberVal-num));
                        $('#' + cart_id + '_product').remove();
                        $('#' + cart_id + '_product_spec').remove();
...
// 检查购物车是否勾选商品
function SubmitOrder_0() {
    layer.msg('请选择要购买的商品！', {time:1000});
}